...
- `Запустить Kwork Chrome` — перезапустить Chrome с твоим обычным профилем, сохраненным входом Kwork и DevTools-портом.
...
Он перезапустит Chrome с твоим обычным профилем `%LOCALAPPDATA%\Google\Chrome\User Data`, портом `9222` и откроет Kwork. Если Chrome уже был открыт без DevTools-порта, скрипт закроет его и откроет заново, чтобы мониторинг видел тот же Kwork-аккаунт.
...
Если Kwork не дает открыть файлы без авторизации, нажми `Запустить Kwork Chrome`: Chrome откроется с твоим обычным профилем, где уже сохранен вход. Сканер сначала пробует обычное скачивание, а при отказе скачивает вложение через эту залогиненную Chrome-сессию.
...
Для Kwork web-режима скрипт использует Chrome через DevTools. Кнопка запускает обычный Chrome-профиль с портом `9222`, чтобы использовать сохраненный вход Kwork.
