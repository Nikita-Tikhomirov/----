...
Если Kwork не дает открыть файлы без авторизации, войди в аккаунт именно в отдельном окне Chrome, которое запускается кнопкой `Запустить Kwork Chrome`. Сканер сначала пробует обычное скачивание, а при отказе скачивает вложение через эту залогиненную Chrome-сессию.
