...
Если к заказу прикреплены ТЗ, документы или скриншоты, инструмент передает в DeepSeek названия, ссылки и по возможности содержимое первых файлов. Сейчас автоматически читаются небольшие `txt`, `md`, `csv`, `html`, `json`, `xml`, `docx` и `pdf` до 2 MB. Картинки и архивы пока не распознаются: AI увидит, что файл есть, и отметит это как риск, не выдумывая детали.
