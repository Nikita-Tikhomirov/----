...
Инструмент читает веб-ленту Kwork по разработке, отбрасывает Bitrix-заказы и проекты с большим числом предложений, отправляет лиды на email и дает готовый текст отклика для ручной отправки.
...
Сейчас фильтр намеренно мягкий по тематике: из Kwork-ленты принимаются почти все заказы, кроме Bitrix/Битрикс. Карточка должна содержать видимый текст `Предложений: N`; если предложений больше `KWORK_MAX_RESPONSES` или число не удалось прочитать, письмо не отправляется. Черновик отклика не просит у заказчика уточнения, доступы или макеты, а сразу предлагает взяться за задачу.
...
Для Kwork web-режима скрипт использует запущенный Chrome через DevTools. Если Chrome уже открыт без DevTools-порта и скрипт не видит карточки, закрой Chrome и снова запусти `watch`: инструмент попробует поднять Chrome с `--remote-debugging-port=9222`.
