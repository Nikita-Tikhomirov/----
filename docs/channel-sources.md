...
Сейчас проект переведен в Kwork-only режим: остальные источники не используются, чтобы не мешать быстрой реакции на новые Kwork-заказы. После получения ссылки инструмент открывает страницу Kwork и пропускает только проекты, где `workerCount` не больше `KWORK_MAX_RESPONSES`.
