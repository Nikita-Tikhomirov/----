...
- A reply cannot hedge with internal uncertainty or judge the buyer's skill level. It must describe a confirmed next step instead of phrases such as "пока исхожу" or "если вы не работали с этим раньше".
- The final readiness sentence uses a direct first-person formulation, never asks the buyer to obtain an answer first.
- If the generated draft violates these rules, the existing repair pass runs. If repair remains invalid, the deterministic fallback is used; it also has no clarification request and selects a safe task description before falling back to the order title.
...
- Conditional promises and assumptions about the buyer's skills are rejected; a fallback does not repeat them from an AI summary.
