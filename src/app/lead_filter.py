...
BLOCKED_PATTERNS = (
    ...
    ("Yandex Kit/Яндекс Кит", re.compile(r"yandex\s*kit|яндекс\s*кит\w*", re.IGNORECASE)),
    ("Timbly/Тимбли", re.compile(r"timbly|тимбл\w*", re.IGNORECASE)),
    ("Webflow", re.compile(r"webflow", re.IGNORECASE)),
    ("Wix", re.compile(r"\bwix\b|викс", re.IGNORECASE)),
    (
        "визуальный конструктор",
        re.compile(r"визуальн\w*\s+конструктор\w*|\bno[ -]?code\b|\bnocode\b", re.IGNORECASE),
    ),
    (
        "iOS-приложение",
        re.compile(
